...
class AutoclassifyTab extends React.Component {
  ...
  async componentDidMount() {
    $rootScope.$on(thEvents.jobClick, () => {
      this.setState({
        loadStatus: "loading",
        errorLines: [],
        selectedLineIds: new Set(),
  ...
  onOpenLogViewer() {
    ...
    window.open(getLogViewerUrl(job.id, $rootScope.repoName, lineNumber));
  }
  ...
  signalFullyClassified() {
    ...
    $rootScope.$emit(thEvents.autoclassifyVerified, { jobs: { [job.id]: job } });
  }
...
export default with$injector(withUser(AutoclassifyTab));

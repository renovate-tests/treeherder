...
class PinBoard extends React.Component {
  ...
  componentDidMount() {
    ...
    $rootScope.$on(thEvents.addRelatedBug, (event, job) => {
  ...
  save() {
    ...
    if (errorFree) {
      ...
      jobs.forEach(job => this.saveClassification(job));
      $rootScope.$emit(thEvents.jobsClassified, { jobs: [...jobs] });

      jobs.forEach(job => this.saveBugs(job));
  ...
  saveClassification(job) {
    // TODO: not saving the classification, just the bugs
  ...
  spaceRemaining() {
    return thPinboardMaxSize - Object.keys(this.props.pinnedJobs).length;
  }
